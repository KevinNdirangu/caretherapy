...
// safe guard around profile placeholder interactions
...
// Contact methods: open Gmail / mailto or WhatsApp instead of copying
document.addEventListener('click', (e) => {
    const method = e.target.closest('.contact-method');
    if (!method) return;

    const email = method.dataset.email;
    const phone = method.dataset.phone;

    if (email) {
        e.preventDefault();
        openEmailComposer(email, 'Inquiry from website', '');
    } else if (phone) {
        e.preventDefault();
...
function openEmailComposer(to, subject = '', body = '') {
    // prefer Gmail web compose (opens in new tab), fallback to mailto
    const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(to)}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    // open Gmail in a new tab; some users may not be signed in — mailto acts as fallback in same tab
    window.open(gmailUrl, '_blank');

    // also prepare mailto (useful for mobile email apps)
    const mailto = `mailto:${encodeURIComponent(to)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    // open mailto in the same tab after a short delay to allow gmail tab to open
    setTimeout(() => {
        window.location.href = mailto;
    }, 500);
}

// Contact form submit: open Gmail compose with prefilled content and keep previous WhatsApp flow
document.addEventListener('DOMContentLoaded', () => {
    const form = document.getElementById('contactForm');
    if (!form) return;

    const therapistName   = 'Connie';
    const therapistNumber = '254787982266';
...
${name}`;
        ...
        // open Gmail compose (new tab) + mailto fallback
        openEmailComposer('conniemuriithi068@gmail.com', subj || 'Website inquiry', body);

        // keep WhatsApp flow: open wa.me with a friendly prefilled message
        const waText =
`Hello ${therapistName},

I hope you’re well. I’d like to schedule a consultation — ${name} (${email}). Message: ${message}`;
        const waUrl = `https://wa.me/${therapistNumber}?text=${encodeURIComponent(waText)}`;
        window.open(waUrl, '_blank');
    ...
    // keep existing service__cta -> WhatsApp behavior (already present earlier) unchanged
    document.querySelectorAll('.service__cta').forEach(btn => {
      btn.addEventListener('click', e => {
        e.preventDefault();
        const card = btn.closest('.service__card');
        const packageName = card ? card.querySelector('.service__title').textContent.trim() : 'package';
